...
//        return $result;
        ...
        if (count($result['cart']) == 0) {
            return $this->sendError('0', 'السلة فارغة');
        } else {
...
//                session(['step' => '0']);
//            }
            if ($this->tempStorage->getSingleTemp('step', -1) == -1)
                $this->tempStorage->createNewSingleTemp('step', 0);
            ...
            if (!empty($all_addresses) and count($all_addresses) > 0) {
                foreach ($all_addresses as $default_address) {
                    if ($default_address->default_address == 1) {
                        $default_address->delivery_phone = auth()->user()->phone;
...
//            }
    ...
    public function thankyou(Request $request)
    {
        ...
        return view("web.thankyou", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
    }
